...
class User extends CI_Controller {
    ...
    public function search($type = NULL) {
        if (!empty($type)) {
            $res = $this->nj_user->user_search_mob($type);
            if (!empty($res)) {
            ...
            $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
            $data["result"] = $this->nj_user->user_search($config["per_page"], $page);
    ...
    public function register() {
		if (!empty($_POST['email'])) {
            ...
            if (empty($dup['cnt'])) {
				...
				$res = $this->db->get_where("users",array("mobile"=>$_POST['mobile']))->row_array();
   ...
   /* public function login() {
        $res = $this->nj_user->login();
        if (!empty($res)) {
            $this->load->helper('string');
            $rand = random_string('alnum', 8) . random_string('numeric', 8) . random_string('alnum', 8) . random_string('numeric', 8);
//            $str = $this->db->query("select user_id from `keys` where user_id = '" . $res['user_id'] . "'");
//            $row = $str->row_array();
//            if (!empty($row)) {
//                $data = array(
//                    "user_id" => $res['id'],
...
public function login() {
        ...
        if (!empty($res['hashkey']) && !empty($res['password'])) {
            ...
            if ($password == $_GET['password']) {
                ...
                $res['password'] = $this->common->decrypt_string($res['password'], $res['hashkey']);
                $this->output
                        ->set_content_type('application/json')
                        ->set_output(json_encode(array("status" => "success", "data" => $res)));
            }
